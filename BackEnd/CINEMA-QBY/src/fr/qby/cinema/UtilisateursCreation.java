...
public class UtilisateursCreation {
	...
	static String createMDP()
	{
		...
		// On prend aléatoirement un charactère parmis les alphanumériques (16 fois pour avoir une bonne longueur de mdp).
		for(int i = 0 ; i < 16 ; ++i )
		{
	...
	public static void main(String[] args) {
		...
		try(FileWriter fw = new FileWriter("utilisateursData.sql", true);
			    BufferedWriter bw = new BufferedWriter(fw);
			    PrintWriter out = new PrintWriter(bw))
			{
				// On écrit dans le fichier à la suite de ce qui existe.
				out.println("INSERT INTO utilisateurs(pseudo,mdp,id_personnes,id_nds)") ;
				out.println("VALUES") ;
				
				// Dans cette boucle on crée de façon aléatoire 1000 utilisateurs.
				Random rand = new Random() ;
			    for(int i = 0 ; i < 1000 ; ++i )
			    {	
			    	// On prend aléatoirement un pseudo dans la liste et on crée un mdp
			    	String tempN = pseudoFutur[rand.nextInt(pseudoFutur.length)] ;
			    	String tempMDP = createMDP() ;
			    	
			    	// On prend aléatoirement un rôle
			    	int tempRole = rolesNDS[rand.nextInt(rolesNDS.length)] ;
